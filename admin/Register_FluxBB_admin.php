...
function Audit_PWG_FluxBB()
{
  ...
  while($row = mysql_fetch_array($result))
    $msg_error_PWG_Dup .= '<br/>'.l10n('Register_FluxBB_Error_PWG_Dup').$row['nbr_dup'].' x '.$row['username'];
  ...
  while($row = mysql_fetch_array($result))
  {
    $msg_error_FluxBB_Dup .= '<br/>'.l10n('Register_FluxBB_Error_FluxBB_Dup').$row['nbr_dup'].' x '.$row['username'];
    ...
    {
      $msg_error_FluxBB_Dup .= '<br/>id:'.$subrow['id'].'='.$subrow['username'].' ('.$subrow['email'].')';
      ...
      if ( !is_adviser() )
      {
        ...
        $msg_error_FluxBB_Dup .= '" title="'.l10n('Register_FluxBB_Del_User').$subrow['username'].'"';
  ...
  while($row = mysql_fetch_array($result))
  {
    $msg_error_Link_Break .= '<br/>'.l10n('Register_FluxBB_Error_Link_Break').$row['pwg_user'].' ('.$row['pwg_mail'].')';
    ...
    if ( !is_adviser() )
    {
      ...
      $msg_error_Link_Break .= '" title="'.l10n('Register_FluxBB_New_Link').$row['pwg_user'].'"';
      $msg_error_Link_Break .= $conf_Register_FluxBB[4]=='false' ?  ' onclick="return confirm(\''.l10n('Are you sure?').'\');" ' : ' ';
      $msg_error_Link_Break .= '><img src="'.Register_FluxBB_PATH.'/icon/link_break.png" alt="'.l10n('Register_FluxBB_New_Link').$row['pwg_user'].'" /></a>';
    }
  }
  ...
  while($row = mysql_fetch_array($result))
  {
    $msg_error_Link_Bad .= '<br/>'.l10n('Register_FluxBB_Error_Link_Del').$row['pwg_user'].' ('.$row['pwg_mail'].')'.' -- '.$row['bb_user'].' ('.$row['bb_mail'].')';
    ...
    if ( !is_adviser() )
    {
      ...
      $msg_error_Link_Bad .= '" title="'.l10n('Register_FluxBB_Link_Del').$row['pwg_user'].' -- '.$row['bb_user'].'"';
      $msg_error_Link_Bad .= $conf_Register_FluxBB[4]=='false' ?  ' onclick="return confirm(\''.l10n('Are you sure?').'\');" ' : ' ';
      $msg_error_Link_Bad .= '><img src="'.Register_FluxBB_PATH.'/icon/link_delete.png" alt="'.l10n('Register_FluxBB_Link_Del').$row['pwg_user'].' -- '.$row['bb_user'].'" /></a>';

      $msg_error_Link_Bad .= ' -- <a href="';

      $msg_error_Link_Bad .= add_url_params($page_Register_FluxBB_admin, array(
        'action' => 'sync_user',
        'username' => $row['pwg_user'],
      ));
      $msg_error_Link_Bad .= '" title="'.l10n('Register_FluxBB_Sync_User').$row['pwg_user'].' --> '.$row['bb_user'].'"';
      $msg_error_Link_Bad .= $conf_Register_FluxBB[4]=='false' ?  ' onclick="return confirm(\''.l10n('Are you sure?').'\');" ' : ' ';
      $msg_error_Link_Bad .= '><img src="'.Register_FluxBB_PATH.'/icon/arrow_switch.png" alt="'.l10n('Register_FluxBB_Sync_User').$row['pwg_user'].' --> '.$row['bb_user'].'" /></a>';
    }
  }
  ...
  while($row = mysql_fetch_array($result))
  {
    $msg_error_Link_Bad .= '<br/>'.l10n('Register_FluxBB_Error_Link_Dup').$row['nbr_dup'].' = '.$row['pwg_user'].' -- '.$row['bb_user'].')';
    ...
    if ( !is_adviser() )
    {
      ...
      $msg_error_Link_Bad .= '" title="'.l10n('Register_FluxBB_Link_Dup').$row['pwg_user'].' -- '.$row['bb_user'].'"';
      $msg_error_Link_Bad .= $conf_Register_FluxBB[4]=='false' ?  ' onclick="return confirm(\''.l10n('Are you sure?').'\');" ' : ' ';
      $msg_error_Link_Bad .= '><img src="'.Register_FluxBB_PATH.'/icon/link_error.png" alt="'.l10n('Register_FluxBB_Link_Dup').$row['pwg_user'].' -- '.$row['bb_user'].'" /></a>';
    }      
  }
  ...
  while($row = mysql_fetch_array($result))
  {
    ...
    {
      $msg_error_Synchro .= '<br/>'.l10n('Register_FluxBB_Error_Synchro').$row['username'];
      ...
      if ( !is_adviser() )
      {
        ...
        $msg_error_Synchro .= '" title="'.l10n('Register_FluxBB_Sync_User').$row['username'].'"';
        $msg_error_Synchro .= $conf_Register_FluxBB[4]=='false' ?  ' onclick="return confirm(\''.l10n('Are you sure?').'\');" ' : ' ';
        $msg_error_Synchro .= '><img src="'.Register_FluxBB_PATH.'/icon/user_refresh.png" alt="'.l10n('Register_FluxBB_Sync_User').$row['username'].'" /></a>';
      }
      ...
      if ( $conf_Register_FluxBB[5] == 'true')
        $msg_ok_Synchro .= '<br/> - '.$row['username'].' ('.$row['pwg_eml'].')'.l10n('Register_FluxBB_Audit_Synchro_OK');
  }
  ...
  while($row = mysql_fetch_array($result))
  {
    $msg_error_PWG2FluxBB .= '<br/>'.l10n('Register_FluxBB_Error_PWG2FluxBB').$row['username'].' ('.$row['mail_address'].')';
    ...
    if ( !is_adviser() )
    {
      ...
      $msg_error_PWG2FluxBB .= '" title="'.l10n('Register_FluxBB_Add_User').$row['username'].'" ';
      $msg_error_PWG2FluxBB .= $conf_Register_FluxBB[4]=='false' ?  ' onclick="return confirm(\''.l10n('Are you sure?').'\');" ' : ' ';
      $msg_error_PWG2FluxBB .= '><img src="'.Register_FluxBB_PATH.'/icon/user_add.png" alt="'.l10n('Register_FluxBB_Add_User').$row['username'].'" /></a>';
    }
  }
  ...
  while($row = mysql_fetch_array($result))
  {
    $msg_error_FluxBB2PWG .= '<br/>'.l10n('Register_FluxBB_Error_FluxBB2PWG').$row['username'].' ('.$row['email'].')';
    ...
    if ( !is_adviser() )
    {
      ...
      $msg_error_FluxBB2PWG .= '" title="'.l10n('Register_FluxBB_Del_User').$row['username'].'"';
      $msg_error_FluxBB2PWG .= $conf_Register_FluxBB[4]=='false' ?  ' onclick="return confirm(\''.l10n('Are you sure?').'\');" ' : ' ';
      $msg_error_FluxBB2PWG .= '><img src="'.Register_FluxBB_PATH.'/icon/user_delete.png" alt="'.l10n('Register_FluxBB_Del_User').$row['username'].'" /></a>';
    }
  }
...
else if ( isset($_GET['action']) and ($_GET['action']=='sync_user') and isset($_GET['username']) and !is_adviser() )
{
  $query = "
    SELECT id AS id_pwg, username, password, mail_address 
  	FROM ".USERS_TABLE."
  	WHERE BINARY username = BINARY '".$_GET['username']."'
  	LIMIT 1
  ;";
  ...
  if (!empty($data))
  {
    FluxBB_Updateuser($data['id_pwg'], $data['username'], $data['password'], $data['mail_address']);
  }
...
else if ( isset($_GET['action']) and ($_GET['action']=='add_user') and isset($_GET['username']) and !is_adviser() )
{
  ...
  if (!empty($data))
    FluxBB_Adduser($data['id'], $data['username'], $data['password'], $data['mail_address']);  
...
if ( isset($_POST['submit']) and !is_adviser() and isset($_POST['FluxBB_prefix']) and isset($_POST['FluxBB_admin']) and isset($_POST['FluxBB_guest']) 
  and isset($_POST['FluxBB_del_pt']) and isset($_POST['FluxBB_confirm']) and isset($_POST['FluxBB_details']) )
{
  $conf['Register_FluxBB'] = $_POST['FluxBB_prefix'].';'.$_POST['FluxBB_admin'].';'.$_POST['FluxBB_guest'].';'.$_POST['FluxBB_del_pt'].';'.$_POST['FluxBB_confirm'].';'.$_POST['FluxBB_details'];
...
$template->assign(array(
    ...
    'FluxBB_ADMIN'            => $conf_Register_FluxBB[1],
    'FluxBB_GUEST'            => $conf_Register_FluxBB[2],
...
if ( isset($_POST['Migration']) and !is_adviser() )
{
  ...
  while ($row = mysql_fetch_array($result))
  {
    if(($row['username'] != $conf_Register_FluxBB[2]) and ($row['username'] != $conf_Register_FluxBB[1]) )
    {
      $msg_Mig_Del_AllUsers .= '<br/> - '.l10n('Register_FluxBB_Mig_Del_User').$row['username'];
  ...
  while ($row = mysql_fetch_array($result))
  {
    if(($row['username'] != 'guest') and ($row['username'] != $conf_Register_FluxBB[1]) )
    {
      $msg_Mig_Add_AllUsers .= '<br/> - '.l10n('Register_FluxBB_Mig_Add_User').$row['username'];

      FluxBB_Adduser($row['id'], $row['username'], $row['password'], $row['mail_address']);
    }
  }
  ...
  if (!empty($row))
  {
    array_push($page['infos'], l10n('Register_FluxBB_Sync_User').$row['username'].'<br/><br/>');
      
    FluxBB_Updateuser($row['id'], $row['username'], $row['password'], $row['mail_address']);
  }
...
if ( isset($_POST['Synchro']) and !is_adviser() )
{
  ...
  if ($msg_error_FluxBB_Dup == '' and $msg_error_PWG_Dup == '')
  {
    ...
    while($row = mysql_fetch_array($result))
    {
      $msg_error_Link_Break .= '<br/>'.l10n('Register_FluxBB_New_Link').$row['pwg_user'].' ('.$row['pwg_mail'].')';
    ...
    while($row = mysql_fetch_array($result))
    {
      $msg_error_Link_Bad .= '<br/>'.l10n('Register_FluxBB_Link_Del').$row['pwg_user'].' ('.$row['pwg_mail'].')'.' -- '.$row['bb_user'].' ('.$row['bb_mail'].')';
    ...
    while($row = mysql_fetch_array($result))
    {
      $msg_error_Link_Bad .= '<br/>'.l10n('Register_FluxBB_Link_Dup').$row['nbr_dup'].' = '.$row['pwg_user'].' -- '.$row['bb_user'].')';
    ...
    while($row = mysql_fetch_array($result))
    {
      if ( ($row['pwg_pwd'] != $row['bb_pwd']) or ($row['pwg_eml'] != $row['bb_eml']) )
      {
        $msg_error_Synchro .= '<br/>'.l10n('Register_FluxBB_Sync_User').$row['username'];
        ...
        if (!empty($data))
          FluxBB_Updateuser($data['id'], $data['username'], $data['password'], $data['mail_address']);
      }
    }
    ...
    while($row = mysql_fetch_array($result))
    {
      $msg_error_PWG2FluxBB .= '<br/>'.l10n('Register_FluxBB_Add_User').$row['username'].' ('.$row['mail_address'].')';
